...
trial_num = 4
OCU = "Jason"
trial_name = f"{OCU}OCU_trial{trial_num}"
df = pd.read_csv(f"delay_modeling/network_test_data/{OCU}OCU/trial{trial_num}.csv")
real_data = df["tcp_rtt (ms)"]
bin_mode = 'auto'
window = 25
online_histogram = de.OnlineHistogram(bin_mode=bin_mode, window=window)

offline_histogram = de.OnlineHistogram(real_data, bin_mode=bin_mode, window=window)
offline_histogram.plot_pmf_cdf(f'delay_modeling/de_animations/{trial_name}_binmode_{bin_mode}_window_{window}.png')
...
axs[1].set_xlim(min(real_data), max(real_data))
...
#                                 bitrate=1800)
ani.save(f'delay_modeling/de_animations/{trial_name}_binmode_{bin_mode}_window_{window}.gif', writer="pillow", fps=1)
