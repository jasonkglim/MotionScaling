...
for test_set_num in range(5):
	...
	test_input = np.linspace(0, 1.5, 100)
	...
	for i in range(len(player_df_train)-2): #, input_latency in enumerate(full_latency_list):
		...
		obs_input = current_obs_df["scale"].values # in N x d
		...
		prediction_dict, kernel_params = model.train_predict(test_input, prediction_df_dense)
		...
		control_scale, policy_choice = policy.optimal_scale(prediction_df_stripped)
